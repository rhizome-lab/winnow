...
- **GML runtime stubs must throw, not silently return wrong values.** When adding a new GML built-in to the runtime: if it requires a real implementation (state mutation, resource management, cross-platform capabilities) and you haven't implemented it, throw `Error("name: not yet implemented")`. Silent returns of 0/""/false are the wrong default — they let game code run with incorrect data and fail silently far from the source. Reserve silent no-ops for: (a) functions that are genuinely no-ops in the target platform (e.g., Steam overlay calls in a browser), (b) functions whose return value is never read. When auditing existing runtime stubs, grep for patterns like `{ return 0; }`, `{ return ""; }`, `{ return false; }`, `{ return -1; }` on functions that are NOT platform-irrelevant no-ops.
