...
- **GML runtime stubs must throw, not silently fail.** When adding a new GML built-in to the runtime: if it requires a real implementation (state mutation, resource management, cross-platform capabilities) and you haven't implemented it, throw `Error("name: not yet implemented")`. Silent returns of 0/""/false are the wrong default — they let game code run with incorrect data and fail silently far from the source. `void {}` is equally wrong for functions with side effects — silently missing functionality is not an acceptable compromise. A game that throws on an unimplemented drawing call is better than a game that silently renders nothing and makes the developer wonder why. Reserve silent no-ops ONLY for functions that are genuinely platform-irrelevant (e.g., `gc_collect` in JS, Steam achievement calls when running offline, OS-specific hints that have no browser equivalent). "The return value is never read" is NOT a valid reason to use a no-op — drawing calls, audio calls, and input calls all have important side effects even though they return void. When auditing existing runtime stubs, grep for patterns like `{ return 0; }`, `{ return ""; }`, `{ return false; }`, `{ return -1; }`, `{}` on functions that are NOT platform-irrelevant no-ops.
