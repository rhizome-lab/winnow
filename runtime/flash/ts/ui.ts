/** Flash UI shim — blocking dialogs via browser native APIs. */
...
export class UiShim {
  ...
  show_choices(prompt: string, choices: string[]): number {
    const message = choices.map((c, i) => `${i + 1}. ${c}`).join("\n");
  ...
  show_number_input(prompt: string, defaultValue: number, min: number, max: number): number {
